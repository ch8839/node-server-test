...
import { Alert, Button, Card, Empty, Spin, Space, Typography } from "antd";
  ...
  useEffect(() => {
    ...
    async function loadDemo() {
      ...
      try {
        ...
        const loader = demoLoaders[key] as
      ...
      <Card className={styles.demoCanvas}>
        <Space direction="vertical" size={12} className={styles.fullWidth}>
          ...
          <div className={styles.demoStage}>
            ...
            {!loading && !error && DemoComponent && <DemoComponent />}
            {!loading && !error && !DemoComponent && <Empty description="暂无演示内容" />}
          </div>
        </Space>
      </Card>
    </div>
